...
#include <BLEServer.h>
#include <BLEScan.h>
#include <esp_eap_client.h> // Updated header file

const char* ssid = "WL_UDESC";
const char* password = "MNC32282633mnc!";
const char* username = "02292380250"; //only if network connection requires
const char* mqtt_server = "172.16.2.103";
const int mqtt_port = 1883;
const char* mqtt_topic = "mauricio/msg";
...
void configureWPA2(){
  esp_wifi_sta_enterprise_set_identity((uint8_t*)username, strlen(username));
  esp_wifi_sta_enterprise_set_username((uint8_t*)username, strlen(username));
  esp_wifi_sta_enterprise_set_password((uint8_t*)password, strlen(password));
